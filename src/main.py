...
class EditorScene(QtWidgets.QGraphicsScene):
    def __init__(self, *args, **kwargs):
        ...
        self.pixel_drawable = PixelDrawableItem()
        self.addItem(self.pixel_drawable)
...
#        self.addItem(self.selected)
        ...
        self.pixel_drawable.setZValue(9999)
...
#    def Selected(self): return self.selected
    ...
    @property
    def PixelDrawable(self): return self.pixel_drawable
...
#        self.__is_draw = False
    ...
    def togglePixel(self):
        img = GraphicView.Scene.Drawable.Pixmap.toImage()
        img.setPixel(self.Selected[0], self.Selected[1], QtGui.QColor(255,0,0) if img.pixel(self.Selected[0], self.Selected[1]) == QtGui.QColor(0,0,0,0) else QtGui.QColor(0,0,0,0))
        GraphicView.Scene.Drawable.Pixmap.fromImage(img)
    def drawPixel(self):
...
class PixelDrawableItem(QtWidgets.QGraphicsRectItem):
    def __init__(self, *args, **kwargs):
        super(self.__class__, self).__init__(*args, **kwargs)
        self.width = 16
        self.height = 16
        self.scale = 32
        self.__pen = QtGui.QPen(QtGui.QColor(0,255,0), 3, QtCore.Qt.DashLine)
        self.__selected = [0, 0]
        self.__draw_color = QtGui.QColor(255,0,0)
        self.__is_draw = False
        self.__is_erase = False
    def paint(self, painter, option, widget):
        painter.setPen(self.__pen)
        painter.drawRect(self.__selected[0]*self.scale, self.__selected[1]*self.scale, self.scale, self.scale)
    @property
    ...
    def move_left(self):
        self.drawPixel()
        self.erasePixel()
        if 0 < self.Selected[0]: self.Selected[0] = self.Selected[0] - 1; self.drawPixel(); self.erasePixel(); GraphicView.viewport().update();
    def move_right(self):
        self.drawPixel()
        self.erasePixel()
        if self.Selected[0] < GraphicView.Scene.Drawable.Pixmap.width()-1: self.Selected[0] = self.Selected[0] + 1; self.drawPixel(); self.erasePixel(); GraphicView.viewport().update();
    def move_up(self):
        self.drawPixel()
        self.erasePixel()
        if 0 < self.Selected[1]: self.Selected[1] = self.Selected[1] - 1; self.drawPixel(); self.erasePixel(); GraphicView.viewport().update();
    def move_down(self):
        self.drawPixel()
        self.erasePixel()
        if self.Selected[1] < GraphicView.Scene.Drawable.Pixmap.height()-1: self.Selected[1] = self.Selected[1] + 1; self.drawPixel(); self.erasePixel(); GraphicView.viewport().update();
    def togglePixel(self):
        img = GraphicView.Scene.Drawable.Pixmap.toImage()
        img.setPixel(self.Selected[0], self.Selected[1], QtGui.QColor(255,0,0) if img.pixel(self.Selected[0], self.Selected[1]) == QtGui.QColor(0,0,0,0) else QtGui.QColor(0,0,0,0))
        GraphicView.Scene.Drawable.Pixmap.fromImage(img)
    ...
    def __update_frame_list(self):
        ...
        FrameListView.viewport().update()
#    def __draw(self):
#        if self.__is_draw: self.drawPixel()
    def keyPressEvent(self, event):
        if   event.key() == QtCore.Qt.Key_Up: self.move_up()
        elif event.key() == QtCore.Qt.Key_Down: self.move_down()
        elif event.key() == QtCore.Qt.Key_Left: self.move_left()
        elif event.key() == QtCore.Qt.Key_Right: self.move_right()
        elif event.key() == QtCore.Qt.Key_Space: self.__is_draw = True; self.drawPixel();
        elif event.key() == QtCore.Qt.Key_Backspace: self.__is_erase = True; self.erasePixel();
        super(self.__class__, self).keyPressEvent(event)
    def keyReleaseEvent(self, event):
        if event.key() == QtCore.Qt.Key_Space: self.__is_draw = False
        elif event.key() == QtCore.Qt.Key_Backspace: self.__is_erase = False
